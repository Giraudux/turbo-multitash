...
interval three_hump_camel(
    const interval &x,
    const interval
        &y) { // Function scaled by factor 600 to avoid fractional coefficients
  return 1200 * pow(x, 2) - 630 * pow(x, 4) + 100 * pow(x, 6) + x * y +
         pow(y, 2);
}
...
interval goldstein_price(const interval &x, const interval &y) {
  return (1 +
          pow(x + y + 1, 2) * (19 - 14 * x + 3 * pow(x, 2) - 14 * y +
                               6 * x * y + 3 * pow(y, 2))) *
         (30 +
          pow(2 * x - 3 * y, 2) * (18 - 32 * x + 12 * pow(x, 2) + 48 * y -
                                   36 * x * y + 27 * pow(y, 2)));
}
...
interval beale(const interval &x, const interval &y) {
  return pow(1.5 - x + x * y, 2) + pow(2.25 - x + x * pow(y, 2), 2) +
