...
const double interval::left(void) const { return bounds[0]; }

const double interval::right(void) const { return bounds[1]; }

double &interval::lb(void) { return bounds[0]; }

double &interval::rb(void) { return bounds[1]; }

bool interval::empty() const { return left() > right(); }
...
inline void round_downward(void) { fesetround(FE_DOWNWARD); }

inline void round_upward(void) { fesetround(FE_UPWARD); }

inline void round_nearest(void) { fesetround(FE_TONEAREST); }
...
inline double round_down(double d) {
  if (d == inf) {
    ...
    return ((d < 0.0) ? psmall * d - numeric_limits<double>::min()
...
inline double round_up(double d) {
  if (d == -inf) {
    ...
    return ((d < 0.0) ? nsmall * d + numeric_limits<double>::min()
